...
class Database {
 ...
 public:
  ...
  Status Get(const ReadOptions&, const std::string_view key, std::string* value) const;
...
static_assert(sizeof(Part) == sizeof(struct iovec));
...
inline uint64_t CalculateCrc(const std::span<const Part, 3>& parts) noexcept {
  ...
  for (const auto& part : parts) {
    XXH64_update(state.get(), part.data, part.len);
  }

  return XXH64_digest(state.get());
}

inline Status LoadFromFile(int fd, void* buf, size_t len, size_t& off) noexcept {
  while (len) {
...
inline std::optional<int> ParseLayoutIndex(std::string_view name) {
  auto pos = name.find("-");
...
inline std::tuple<size_t, Status> ReadEntryImpl(const int fd, const size_t offset, const bool check_crc,
  ...
  if (check_crc) {
    const std::array parts{
        ...
        detail::Part{.data = value.data(), .len = value.size()},
        detail::Part{.data = key.data(), .len = key.size()},
    };
...
void Database::FileInfo::CloseFile(bool data_sync) {
  int prev_fd = -1;
...
Status Database::Initialize() {
  ...
  for (const auto& entry : std::filesystem::directory_iterator(base_path_)) {
    ...
    auto index = detail::ParseLayoutIndex(entry.path().filename().string());
...
template <typename K>
void Database::WaitKeyUnlocked(const std::string_view key, K& lock) const {
  std::unique_lock lock_lock(lock_mutex_);
...
Status Database::ReadValue(const ReadOptions& options, const Record& record, std::string& value) const {
  ...
  if (options.verify_checksums) {
    ...
    // Load value.
    return detail::LoadFromFile(record.file->fd, value.data(), value.size(), offset);
  }
}
